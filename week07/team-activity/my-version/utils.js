...
function renderOneHike(hike) {
   ...
   item.innerHTML = ` <h2 id="hikeVenue">${hike.name}</h2>
...
// added by M.Sierra
function showHikeDetails(hike, item, parent) {
...
function showCommentInput(hike, item) {
   let divInput = document.createElement("div");
   divInput.setAttribute("id", "hikeComments");
   divInput.innerHTML = `
                  <div>
                     <h3>Add a comment</h3>
                     <textarea id="commentEntry" name="comments" rows="4" cols="50"
                      placeholder="Enter comments for this hike"></textarea><br>
                     <button id="commentSubmit">SubmitComment</button>
                  </div>
   `;
   if (item.querySelector("#hikeComments") === null) {
      item.appendChild(divInput);
   }
   return item;
}
